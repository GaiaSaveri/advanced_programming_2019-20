...
//we want to do different things for different animals
//but types are resolved runtime
void print_animal(const Animal& a) noexcept {
  a.info();
  a.speak();

  // dynamic_cast is used to know the real type of the object
  //works with pointers and references bc we are in polymorphism
  //dynamic cast is usefull when we know the type we want
  //we for sure have runtime penalty
  if (dynamic_cast<const DangerousSnake*>(&a))
    std::cout << "call 911...\n";
}

//with templates we can do the same thing
//but in a very different way
//we have code duplication but not runtime penalty
//with templates if we are carefull everything can be inlined
